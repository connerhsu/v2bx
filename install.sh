...
EOF
    ...
    if [[ ! -f /etc/V2bX/route.json ]]; then
        # 强制创建一个空的路由规则，避免内置审计规则
        echo '{"rules": []}' > /etc/V2bX/route.json
    fi
    ...
    if [[ $first_install == true ]]; then
        ...
        if [[ $if_generate == [Yy] ]]; then
            ...
            # --- 核心修改：去除生成配置后的审计内容 ---
            if [[ -f /etc/V2bX/config.json ]]; then
                echo -e "${yellow}正在清理配置文件中的审计规则引用...${plain}"
                # 删除包含 RuleListPath 的所有行（去除远程审计列表）
                sed -i '/RuleListPath/d' /etc/V2bX/config.json
                # 再次尝试重启以应用无审计配置
                if [[ x"${release}" == x"alpine" ]]; then
                    service V2bX restart >/dev/null 2>&1
                else
                    systemctl restart V2bX >/dev/null 2>&1
                fi
                echo -e "${green}审计内容清理完成！${plain}"
            fi
        fi
    fi
